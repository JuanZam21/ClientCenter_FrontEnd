...
import ModalResponseCredito from "./response/CreditosResponse";
...
function Creditos() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${config.baseUrl}/api/credito/questions`);
  ...
  const sendSolicitud = async (e) => {
    ...
    try {
      const send = await fetch(`${config.baseUrl}/api/credito`, {
        method: "POST",
        headers: {
  ...
  return (
    <div>
      {resData && <ModalResponseCredito creditoData={resData} isOpen={true} />}
      <NavBar />
      {error.isError && (
      ...
      <section className="flex justify-center">
        <section className="p-5 w-full">
          <article className="px-10">
            <h1 className="text-[#ed1218] text-4xl  text-bold">Creditos</h1>
